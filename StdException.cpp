...
BOOL
StdException::GetErrorMessage(LPCTSTR p_error,UINT p_maxSize,PUINT p_helpContext /*= NULL*/)
{
  // Reset help context
  if(p_helpContext)
  {
  ...
  // Copy it out
  _tcsncpy_s((LPTSTR)p_error,(size_t)p_maxSize,(LPTSTR)error.GetString(),error.GetLength() + 1);
  return TRUE;
}
